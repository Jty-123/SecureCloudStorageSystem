...
//        String dateStr = "2019-01-03 10:59:27";
        ...
        HashMap<String, Object> saveresult = databaseser.save(username,password,role,date);
    ...
    public Object userlist(@RequestBody JSONObject json, HttpServletRequest servletrequest) throws ParseException {
        ...
        String timestamp = json.getString("timestamp");
        JSONObject request = json.getJSONObject("request");
        String userrole = request.getString("userrole");
        JSONObject keywords = request.getJSONObject("keywords");
        String username = "";
        if(null!=keywords){
            username = keywords.getString("username");
        }
        System.out.println("username is " + username);
        HashMap<String, Object> findmap =databaseser.findAll(username,userrole);
    ...
    public Object usermodify(@RequestBody JSONObject json, HttpServletRequest servletrequest) throws ParseException {
        ...
        RegisterUser ret =(RegisterUser) databaseser.modifyuser(username,email);
        JSONObject retjsonobj = new JSONObject();
        retjsonobj.put("username",ret.getName());
        retjsonobj.put("role",ret.getRole());
        retjsonobj.put("date",ret.getDate());
        retjsonobj.put("email",ret.getEmail());
