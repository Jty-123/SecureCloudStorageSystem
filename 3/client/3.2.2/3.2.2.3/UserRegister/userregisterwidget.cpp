...
void UserRegisterWidget::checkUsernameExistence(){
    ...
    network_request.setUrl(QUrl("https://127.0.0.1:9443/checkusernameexistence"));
...
void UserRegisterWidget::slot_username_linedit_focussed(bool stat){//输入框获取或失去鼠标焦点出发的槽函数
...
void UserRegisterWidget::slot_password1_linedit_focussed(bool stat){//输入框获取或失去鼠标焦点出发的槽函数
    qDebug()<<Q_FUNC_INFO<<"stat is "<<stat;
    capskeystat = stat;
    if(stat == true){
        if(CapsButtonState()){
...
void UserRegisterWidget::slot_password2_linedit_focussed(bool stat){//输入框获取或失去鼠标焦点出发的槽函数
    qDebug()<<Q_FUNC_INFO<<"stat is "<<stat;
    capskeystat = stat;
    if(stat == true){
        if(CapsButtonState()){
...
bool UserRegisterWidget::checkUsernameRegular(){
    ...
    if(nullptr==username||username.length()<=0){
        ...
        if(!usernameRange->exactMatch(username)){
            ...
            ui->username_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">用户名不规范</span></p></body></html>");
            return false;
        }
        if(username.length()<8||username.length()>20){
            ui->username_notice_label->show();
            ui->username_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">用户名长度需要8-20位</span></p></body></html>");
            return false;
        }
        if(!lowercase->exactMatch(username)&&
            ...
            ui->username_notice_label->show();
            ui->username_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">用户名需包含字母</span></p></body></html>");
            return false;
        }else if(!numbercase->exactMatch(username)){
            ui->username_notice_label->show();
            ui->username_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">用户名需包含数字</span></p></body></html>");
            return false;
        }else{
            ui->username_notice_label->hide();
            return true;
...
bool UserRegisterWidget::checkPassword1Regular(){
    ...
    if(nullptr==password1||password1.length()<=0){
        ...
        if(!passwordRange->exactMatch(password1)){
            ...
            ui->password1_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">口令不规范</span></p></body></html>");
            return false;
        }
        if(password1.length()<10||password1.length()>20){
            ui->password1_notice_label->show();
            ui->password1_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">口令长度需要10-20位</span></p></body></html>");
            return false;
        }
        int matchnum = 0;
        ...
        if(matchnum>=3){
            ...
            ui->password1_notice_label->show();
            ui->password1_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">口令需包含数字、大写字母、小写字母、特殊符号中至少三种</span></p></body></html>");
            return false;
        }
...
void UserRegisterWidget::userRegister(QString username, QString password,QString role){
    ...
    ur_request.setUrl(QUrl("https://127.0.0.1:9443/userregister"));
...
void UserRegisterWidget::slot_replyFinished(QNetworkReply* reply){
    ...
    if (nullptr != reply) {
        ...
        if(ifexist){
            ui->username_notice_label->show();
            ui->username_notice_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">用户名已存在</span></p></body></html>");
        }else{
            ui->username_notice_label->show();
            ui->username_notice_label->setText("<html><head/><body><p><span style=\" color:##00aaff;\">用户名不存在，可以注册</span></p></body></html>");
...
void UserRegisterWidget::slot_ur_replyFinished(QNetworkReply* reply){
    ...
    if (nullptr != reply) {
        ...
        if(result.compare("success")==0&&
            ...
            ui->registerresult_label->setText("<html><head/><body><p><span style=\" color:#00aaff;\">注册成功</span></p></body></html>");
        }else{
            ui->registerresult_label->show();
            ui->registerresult_label->setText("<html><head/><body><p><span style=\" color:#ff0000;\">"+message+"</span></p></body></html>");
