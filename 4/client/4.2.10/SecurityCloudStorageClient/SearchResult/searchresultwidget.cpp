...
void SearchResultWidget::data_search(
    ...
    if(keywords.contains("|")){
        condition = "OR";
        keywordslist = keywords.split(QRegExp("\\|+"));//将关键词按照字符"|"进行分割形成字符串数组，其中\\|表示字符"|"，+表示匹配前面的子表达式一次或多次
    }else{
        keywordslist = keywords.split(QRegExp("&+| +"));//将关键词按照字符"&"或空格进行分割形成字符串数组，其中&表示字符"&"，+表示匹配前面的子表达式一次或多次，|表示或
    }
    keywordslist.removeDuplicates();//去重
    keywordslist.removeOne("");//去除空字符
    ...
    if(0==this->mode.compare("filename")){
        ...
        for(int i = 0; i< keywordslist.size();++i)
        {
            ...
            std::string out = YunLock_EncryptKeyword(tmp.toStdString());
