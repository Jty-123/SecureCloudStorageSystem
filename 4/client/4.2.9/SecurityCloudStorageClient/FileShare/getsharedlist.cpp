#include "getsharedlist.h"

GetSharedList::GetSharedList(QObject *parent) : QObject(parent)
{

}
GetSharedList::GetSharedList(QTableWidget * tablewidget,QObject *parent) : QObject(parent)
{
    this->tablewidget=tablewidget;
}


void GetSharedList::get_data(){
...
void GetSharedList::slot_replyFinished(QNetworkReply* reply){
    QByteArray ret_data;
...
void GetSharedList::refreshsharelistview(QList<DataShareEntity*> * sharelist){
    ...
    if (nullptr != sharelist&&sharelist->size() > 0) {
        ...
        while (iterater.hasNext()) {
            ...
            QPushButton *pushbutton = new QPushButton("下载");
            connect(pushbutton,SIGNAL(clicked()),entity,SLOT(download_btn_clicked()));
...
QList<DataShareEntity*> * GetSharedList::parse_json_array(QJsonArray &filelistarray ){
    ...
    for(int i = 0; i< filelistarray.size(); i++)
    {
        ...
        if(pobj.contains("shareusername"))
        {
            shareusername = pobj.value("shareusername").toString();
            shareentity->setshareusername(shareusername);
        }
        QString filename;
...
void GetSharedList::slot_sslErrors(QNetworkReply *reply, const QList<QSslError> &errors){
    QSslCertificate sslcert = errors[0].certificate();
    reply->ignoreSslErrors();
}
void GetSharedList::slot_provideAuthenication(QNetworkReply* reply, QAuthenticator* authenticator){

}
void GetSharedList::slot_NetWorkError(QNetworkReply::NetworkError){
...
QString GetSharedList::convert_size(qint64 size){
    QString prefix;
    double speed = size;
    if(speed<=900){
        prefix = "B";
